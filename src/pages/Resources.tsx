...
const Resources = () => {
  return (
    <div className="bg-white min-h-screen flex flex-col">
      <Header />
      {/* Banner */}
      <div className="relative w-full h-56 md:h-64 flex items-center justify-center bg-gray-200">
          ...
          {pdfs.map((pdf, idx) => (
            <a key={idx} href={pdf.link} target="_blank" rel="noopener noreferrer" className="block w-72 bg-white rounded shadow hover:shadow-lg transition overflow-hidden border border-gray-200">
              <div className="relative">
                ...
                <img src="/public/placeholder.svg" alt="PDF" className="absolute bottom-2 right-2 w-10 h-10" />
              </div>
          ...
          {howToVideos.map((vid, idx) => (
            <div key={idx} className="bg-white rounded shadow hover:shadow-lg transition overflow-hidden border border-gray-200 flex flex-col items-center">
              <div className="relative w-full">
                <img src="/public/placeholder.svg" alt={vid.title} className="w-full h-44 object-cover" />
                <img src="/public/placeholder.svg" alt="Play" className="absolute inset-0 m-auto w-14 h-14" style={{top: '0', left: '0', right: '0', bottom: '0'}} />
              </div>
